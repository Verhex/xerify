...
explicit save flag and the exact root `node_modules/xerify-cli` location. A package manager could hoist
...
Empty `providers` keeps the built-in `codex`, `claude`, and `jev` adapters. Add only the transports you use.
For example:
...
may replace it with `../node_modules/xerify-cli/schemas/config.schema.json` for offline completion.

Config is strict. Generic `token`, `secret`, silent `defaultModel`, and credential fields on command
or CLI adapters are rejected. Direct `jev`, `openai-api`, `anthropic-api`, and `openai-compatible` adapters
...
`--timeout 0` or `limits.timeoutMs: 0` explicitly disables the provider lifecycle deadline. Cancellation and byte bounds remain active; the default stays 120000 ms.

The environment equivalent is `XERIFY_TIMEOUT_MS=0`; a blank value is rejected. Byte limits must remain positive.
