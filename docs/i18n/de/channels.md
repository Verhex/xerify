...
npm-Paket `xerify-cli`; es hostet den Code von Xerify nicht selbst. Die `server.json` im
...
`npx -y --package=xerify-cli@<version> xerify mcp stdio` der deterministische Fallback. Siehe den
