...
Jev is included as the default `jev` adapter (`typesafe` provider). `--to jev` selects `typesafe:jev-latest`; `--to jev:MODEL_ID` selects an explicit model. Jev supports `verify` only. Its probabilities, confidence, returned model, and policy appear in optional `decision` metadata. Below the configured thresholds, Xerify returns `unclear`. Explanations and citations are not generated by Jev.

[Jev / 0.3.0](jev.md)

## Cursor verification formatting

Xerify invokes the separately installed Cursor Agent CLI executable `agent`; it does not install Cursor or call a Cursor model API directly. The adapter uses `-p --mode ask --sandbox enabled --output-format json` in a temporary workspace. `--output-format json` formats the CLI envelope; its `result` field is free-form assistant text, not a schema-constrained verdict. A successful process and reported token usage can therefore still produce `INVALID_PROVIDER_RESPONSE`. See the [official output contract](https://cursor.com/docs/cli/reference/output-format).

A synthetic diagnostic on 2026-09-18 reproduced this: the envelope parsed, while its inner response was prose. The installed CLI was `2026.09.15-d2fe57e`, different from the historical benchmark version. The adapter now appends a concrete JSON output example and a machine-output reminder for `verify` only. It still sends the full prompt on stdin, leaves `ask`/`request` unchanged, and never extracts a verdict from prose or strips fences. This is prompt guidance, not native schema enforcement; `structuredOutput` remains false. The CLI ignores stdin when a positional prompt is supplied, so a generic argv instruction plus evidence on stdin is not a valid workaround.

Cursor is excluded from the active OpenAI/Anthropic/Jev benchmark. Small integration diagnostics after a prompt change must be reported separately, not substituted for historical measurements. Keep strict schema rejection and the Cursor adapter's regression tests even while excluding Cursor from the product comparison.

After the change, four fixed live checks through Xerify core and the real Cursor adapter passed: `indirection` → confirmed (22,452 ms), `sql` → refuted (8,026 ms), `missing` → unclear (14,592 ms), `injection` → refuted (11,183 ms). All had failure null. Before these checks, three diagnostic calls compared the original prompt (prose), an argv-plus-stdin experiment (missing evidence; not adopted), and the JSON-example candidate (valid SQL refutation). None enter the benchmark. Four passing checks demonstrate the tested integration paths, not universal formatting reliability; native schema enforcement is still absent.
