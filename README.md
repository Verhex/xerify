...
<p align="center"><strong>Verify before you trust.</strong></p>

<p align="center">
  <img src="https://raw.githubusercontent.com/Verhex/xerify/main/assets/readme/xerify-verification-flow.gif?v=0.3.0" alt="Xerify 0.3.0: LLM verification or Jev typed decisions. One target per run; illustrative probabilities and verdicts." width="960">
</p>

Xerify is a model-independent verification layer for AI systems. It checks claims against bounded evidence through a different invocation provider, using LLM verifiers or Jev typed decisions. It can use provider CLIs already authenticated on your machine, direct APIs, or an explicitly configured executable. The same core and schemas power the CLI, library, local STDIO MCP server, and Streamable HTTP MCP server.
...
> Xerify 0.3.0 adds Jev typed decisions alongside LLM verification. An owner-approved campaign check and ten additional live Jev scenarios matched expectations. See the [scenario results](docs/jev-testing.md). This is integration evidence, not a model-accuracy benchmark.
...
<https://github.com/Verhex/xerify/blob/main/docs/README.md>.
...
The default config exposes the official `codex` and `claude` CLI adapters and the TypeSafe `jev` decision adapter. Additional built-ins are
...
Use `--to provider:model` to select a model explicitly. For Jev verification, `--to jev` is a documented shorthand for `typesafe:jev-latest`; pin `--to jev:jev-1.13.0` when evaluating a fixed model version.
...
[contribution guide](https://github.com/Verhex/xerify/blob/main/CONTRIBUTING.md); the public
...
Xerify is designed, built, and maintained by **[Verhex](https://github.com/Verhex)**.
...
[Cross-provider benchmark](docs/benchmark.md) · [0.3.0 campaign](docs/launch-0.3.0.md)
